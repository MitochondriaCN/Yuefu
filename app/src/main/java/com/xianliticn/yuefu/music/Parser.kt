...
class Parser {
    fun generateMidiEvents(musicXmlDoc: Document): List<MidiEvent> {
        val midiEvents = mutableListOf<MidiEvent>()

        // MusicXML 的根节点通常是 <score-partwise>
        val root = musicXmlDoc.rootElement

        // 获取第一个声部 (part) 的所有小节 (measure)
        // 注意：这里简化处理，只取第一个 part。如果有多个 part 需要额外循环处理。
        val part = root.element("part") ?: return emptyList()
        val measures = part.elements("measure")

        // 累计时间（纳秒），用于计算每个音符的绝对开始时间
        var currentTimeNano: Long = 0

        // 假设默认速度 (BPM)，如果 XML 中没有定义。例如 120 BPM。
        var currentBpm = 120
        ...
        for (measure in measures) {
            ...
            // 1. 检查 attributes 更新 (如 divisions, key, time signature)
            val attributes = element.element("attributes")
            if (attributes != null) {
                attributes.elementText("divisions")?.let {
                    divisions = it.toInt()
                }
                // 这里也可以解析 <clef>, <key>, <time> 等
            }

            // 2. 检查 direction 更新 (如 tempo/BPM)
            ...
            for (noteObj in notes) {
                ...
                // 简化: duration * (60_000_000_000 / (divisions * BPM))
                // 注意防止除零
